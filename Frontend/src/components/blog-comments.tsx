...
import { dummyBlogCommentsBySlug } from "@/lib/dummy-blogs";
...
function displayName(comment: BlogComment) {
  if (comment.author) return `${comment.author.firstName} ${comment.author.lastName}`.trim();
  return comment.guestName || "Guest";
}

export function BlogComments({ slug, isDummy = false }: { slug: string; isDummy?: boolean }) {
  ...
  const loadComments = async () => {
    ...
    try {
      if (isDummy) {
        const dummy = (dummyBlogCommentsBySlug[slug] || []) as BlogComment[];
        setComments(dummy);
        setTotal(dummy.length);
        return;
      }
      const result = await getBlogComments(slug, { page: 1, limit: 50 });
      if (result.comments.length === 0 && (dummyBlogCommentsBySlug[slug]?.length || 0) > 0) {
        const dummy = (dummyBlogCommentsBySlug[slug] || []) as BlogComment[];
        setComments(dummy);
        setTotal(dummy.length);
      } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const created = await createBlogComment(slug, {
        content: content.trim(),
      ...
      <div className="mb-7 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {total} comment{total === 1 ? "" : "s"} on this blog
          </p>
        </div>
