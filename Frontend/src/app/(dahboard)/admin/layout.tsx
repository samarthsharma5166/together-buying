      ...
      <Sidebar collapsible="icon">
        <SidebarHeader className="border-b border-sidebar-border/50">
          <div className="flex items-center gap-3 px-0 py-1.5">
            <div className="flex h-8 w-9 shrink-0 items-center justify-center rounded-lg bg-[#e34b32] text-white font-black shadow-md">
              A
            </div>
                    ...
                    <SidebarMenuItem key={item.href}>
                      <SidebarMenuButton asChild isActive={isActive} tooltip={item.label}>
                        <Link href={item.href} className="flex items-center w-full gap-3">
                          <Icon className={cn("size-4 shrink-0", isActive ? "text-shadow-sidebar-primary-foreground" : "text-sidebar-foreground/70")} />
              ...
              <div className="flex items-center justify-between gap-2 p-1.5 group-data-[collapsible=icon]:justify-center">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0 group-data-[collapsible=icon]:hidden">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-wider text-sidebar-foreground/50">{user.role}</p>
                  </div>
                </div>
                <button
      ...
      <div className="flex flex-col flex-1 min-w-0 bg-slate-50">
        <header className="flex h-16 items-center justify-between border-b border-slate-200/80 bg-white px-4 shrink-0">
          ...
          <Link href="/" className="flex items-center gap-2 sm:hidden">
            ...
            <span className="font-display text-sm font-black text-[#111111]">Admin Console</span>
          </Link>
        </header>
