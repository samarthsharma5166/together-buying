...
import {
  dummyBlogs,
  getDummyBlogBySlug,
  getDummySimilarBlogs,
} from "@/lib/dummy-blogs";
...
export async function generateStaticParams() {
  const { blogs: apiBlogs } = await getBlogs({ page: 1, limit: 100 });
  const blogs = apiBlogs.length > 0 ? apiBlogs : dummyBlogs;
...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const similar = apiSimilar.length > 0 ? apiSimilar : getDummySimilarBlogs(slug, 3);
      ...
      <div className="border-b border-slate-200/80 bg-white/80 backdrop-blur">
        <div className="container-shell flex flex-wrap items-center justify-between gap-3 py-3.5 text-sm">
          <nav className="flex items-center gap-1.5 font-bold text-slate-500">
            ...
            <Link href="/blogs" className="transition hover:text-[#e34b32]">
              Blogs
            </Link>
            <ChevronRight size={14} className="text-slate-300" />
            <span className="max-w-[180px] truncate text-slate-800 sm:max-w-xs">{blog.category}</span>
          </nav>
          <Link
            href="/blogs"
        ...
        <header className="border-b border-slate-200/70 bg-white">
          <div className="container-shell max-w-4xl py-10 md:py-14">
            <div className="mb-5 flex flex-wrap items-center gap-3 text-xs font-black uppercase tracking-[0.18em]">
              <span className="rounded-full bg-[#e34b32] px-3.5 py-1.5 text-white">{blog.category}</span>
              <span className="text-slate-400">·</span>
              <span className="inline-flex items-center gap-1.5 normal-case tracking-normal text-slate-500">
                <CalendarDays size={14} /> {formatDate(blog.createdAt)}
              </span>
              <span className="text-slate-400">·</span>
              <span className="inline-flex items-center gap-1.5 normal-case tracking-normal text-slate-500">
                <Clock size={14} /> {blog.readTimeMin} min read
              </span>
            </div>
            ...
            <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-slate-100 pt-6">
              ...
              <ShareButton title={blog.title} text={blog.excerpt || blog.title} />
            </div>
          </div>
        </header>
            ...
            <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-14">
              <div className="min-w-0">
                <div className="mx-auto max-w-2xl lg:mx-0 lg:max-w-none">
                  <div>{renderBlogBody(blog.content)}</div>
                  ...
                  <BlogComments slug={slug} isDummy={isDummy} />
                </div>
              </div>
              ...
              <aside className="lg:pt-1">
                <div className="sticky top-24 space-y-5">
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">In this blog</p>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                      ...
                      <div className="mt-4 space-y-4">
                        {similar.slice(0, 3).map((item) => (
                          <Link key={item.id} href={`/blogs/${item.slug}`} className="group flex gap-3">
